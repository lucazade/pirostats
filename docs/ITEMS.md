...
| `gpu_amd_usage` | panel · tooltip | AMD GPU utilization, %. | `Radeon usage:  9%` |
| `gpu_amd_mem_usage` | panel · tooltip | AMD VRAM occupancy (used/total), %. | `Radeon vram usage:  11%` |
| `gpu_amd_temp` | panel · tooltip | AMD GPU temperature (the `edge` sensor). | `Radeon temp:  48°C` |
| `gpu_amd_fan_speed` | panel · tooltip | AMD fan in RPM; `off` when idle at 0. | `Radeon fan speed:  off` |
| `gpu_amd_freq` | panel · tooltip | AMD GPU clock frequency (`sclk`). | `Radeon freq:  144 MHz` |
...
| `wifi_ant1` / `wifi_ant2` | panel · tooltip | Signal of each antenna, dBm. `wifi_ant2` appears only on a radio with a second antenna. | `Antenna 1:  -57 dBm` |
| `wifi_tx` / `wifi_rx` | panel · tooltip | Link rate per direction, with its MCS (modulation index) and NSS (spatial streams) in the middle. The panel shows one at a time, rotating every 5 s: `1153`, `MCS5`, `NSS2`. | `Wifi TX: MCS 5 NSS 2  1152.8 Mbit/s` |

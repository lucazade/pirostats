...
- **Wi-Fi data comes from nl80211, not `iw`** (`src/nl80211.py`): a stdlib generic-netlink socket held in `DaemonState`, ~0.2ms per station dump against ~3ms for forking `iw`. The SSID rides the 10s `net_info` cache; the live link (signal, per-antenna dBm, tx/rx rate + MCS/NSS) is the `wifi_link` capability, read every poll. Replies are matched on the netlink sequence number, so a late answer can't be taken for the current request.
